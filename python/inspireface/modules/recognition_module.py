...
class FaceRecognitionModule(object):
    ...
    def set_search_threshold(self, search_threshold: float):
        self.search_threshold = search_threshold
        ret = HFFeatureHubFaceSearchThresholdSetting(self.engine.handle, search_threshold)
        if ret != 0:
    ...
    def extract_feature_from_token(self, stream: CameraStream, token: HFFaceBasicToken) -> np.ndarray:
        feature_length = HInt32()
        ret = HFGetFeatureLength(self.engine.handle, byref(feature_length))
        if ret != 0:
            raise Exception("error")
        feature = (HFloat * feature_length.value)()
        ret = HFFaceFeatureExtractCpy(self.engine.handle, stream.handle, token, feature)
        if ret != 0:
    ...
    def face_comparison1v1(self, face1: FaceInformation, face2: FaceInformation) -> float:
        ...
        ret = HFFaceComparison(self.engine.handle, feats[0], feats[1], HPFloat(comparison_result))
        if ret != 0:
    ...
    def face_register(self, face_identity: FaceIdentity):
        ret = HFFeatureHubInsertFeature(self.engine.handle, face_identity.to_ctypes())
        if ret != 0:
    ...
    def face_search(self, data) -> SearchResult:
        ...
        feature = HFFaceFeature(size=HInt32(vector.size), data=vector.ctypes.data_as(HPFloat))
        confidence = HFloat()
        most_similar = HFFaceFeatureIdentity()
        ret = HFFeatureHubFaceSearch(self.engine.handle, feature, HPFloat(confidence),
                                            PHFFaceFeatureIdentity(most_similar))
        if ret != 0:
    ...
    def face_remove(self, custom_id: int):
        ret = HFFeatureHubFaceRemove(self.engine.handle, custom_id)
        if ret != 0:
            raise Exception(f"Failed to remove face: {ret}")

    def face_update(self, face_identity: FaceIdentity):
        ret = HFFeatureHubFaceUpdate(self.engine.handle, face_identity.to_ctypes())
        if ret != 0:
    ...
    def get_face_identity(self, custom_id: int) -> FaceIdentity:
        ...
        ret = HFFeatureHubGetFaceIdentity(self.engine.handle, custom_id, PHFFaceFeatureIdentity(identify))
        if ret != 0:
    ...
    def get_face_count(self) -> int:
        count = HInt32()
        ret = HFFeatureHubGetFaceCount(self.engine.handle, HPInt32(count))
        if ret != 0:
    ...
    def view_table_in_terminal(self):
        ret = HFFeatureHubViewDBTable(self.engine.handle)
        if ret != 0:
