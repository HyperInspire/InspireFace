...
def case_face_detection_image(resource_path, image_path):
    ...
    session = ifac.InspireFaceSession(opt, HF_DETECT_MODE_ALWAYS_DETECT, detect_pixel_level=640)
    session.set_track_preview_size(640)
    ...
    for idx, face in enumerate(faces):
        ...
        cv2.drawContours(draw, [box], 0, (180, 170, 29), 10)

        # Draw landmarks
        lmk = session.get_face_dense_landmark(face)
        for x, y in lmk.astype(int):
            cv2.circle(draw, (x, y), 0, (220, 100, 0), 15)
