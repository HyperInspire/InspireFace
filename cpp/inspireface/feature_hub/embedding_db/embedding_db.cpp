...
namespace inspire
{
    ...
    EmbeddingDB::EmbeddingDB(const std::string &dbPath, size_t vectorDim,
                             const std::string &distanceMetric, IdMode idMode)
        : vectorDim_(vectorDim), tableName_("vec_items"), idMode_(idMode)
    {
        ...
        std::string createTableSQL =
            "CREATE VIRTUAL TABLE IF NOT EXISTS " + tableName_ +
    ...
    std::vector<FaceSearchResult> EmbeddingDB::SearchSimilarVectors(
        const std::vector<float> &queryVector, size_t top_k, float keep_similar_threshold, bool return_feature)
    {
        std::lock_guard<std::mutex> lock(dbMutex_);
        ...
        std::string sql;
        if (return_feature) {
            sql = "SELECT rowid, embedding, 1.0 - distance as similarity "
                  "FROM " + tableName_ + " "
                  "WHERE embedding MATCH ? "
                  "ORDER BY distance "
                  "LIMIT ?";
        } else {
            sql = "SELECT rowid, 1.0 - distance as similarity "
                  "FROM " + tableName_ + " "
                  "WHERE embedding MATCH ? "
                  "ORDER BY distance "
                  "LIMIT ?";
        ...
        results.erase(std::remove_if(results.begin(), results.end(),
                                     [keep_similar_threshold](const FaceSearchResult &result)
                                     { return result.similarity < keep_similar_threshold; }),
                      results.end());
