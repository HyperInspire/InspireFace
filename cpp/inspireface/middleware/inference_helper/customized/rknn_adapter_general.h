 ...
 * @ingroup NeuralNetwork
 */
class RKNNAdapterGeneral {
public:
    // 禁止拷贝
    RKNNAdapterGeneral(const RKNNAdapterGeneral &) = delete;
    RKNNAdapterGeneral &operator=(const RKNNAdapterGeneral &) = delete;
    RKNNAdapterGeneral() = default;
    ...
    int Initialize(const char *model_path) {
        ...
        int ret = rknn_init(&rk_ctx_, model_data, model_data_size, 0, NULL);
     ...
     * @return 返回初始化结果
     */
    int Initialize(void *model_data, unsigned int model_size) {
        /* Create the neural network */
        int model_data_size = 0;
        //        LOGD("Read models size: %f MB", round(float(model->modelsize.caffemodel_size) / (1024 * 1024)));
        INSPIRE_LOGI("The neural network is being initialized...");
        int ret = rknn_init(&rk_ctx_, model_data, model_size, 0, NULL);
        //        LOG_INFO("RKNN Init ok.");
        if (ret < 0) {
            INSPIRE_LOGE("rknn_init fail! ret=%d", ret);
            return -1;
    ...
    int Initialize(const unsigned char *model_data, const unsigned int model_size) {
        /* Create the neural network */
        INSPIRE_LOGI("The neural network is being initialized...");
        int ret = rknn_init(&rk_ctx_, (void *)model_data, model_size, 0, NULL);
     ...
     * @return 返回输入状态
     */
    Status SetInputData(const int index, uint8_t *data, rknn_tensor_type type = RKNN_TENSOR_UINT8, rknn_tensor_format format = RKNN_TENSOR_NHWC) {
        if (index < input_tensors_.size()) {
            input_tensors_[index].index = 0;
            input_tensors_[index].type = RKNN_TENSOR_UINT8;
            input_tensors_[index].size = input_attrs_[0].dims[1] * input_attrs_[0].dims[2] * input_attrs_[0].dims[3];
